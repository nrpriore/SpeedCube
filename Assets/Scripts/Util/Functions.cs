...
using UnityEngine.UI;				// For UI classes
...
public static class Functions {
	...
	// Loops through parent and types of children to set alpha
	public static void SetAlphaWithChildren(Image parentImage, float value) {
		SetImageAlpha(parentImage, value);

		foreach(Transform child in parentImage.transform) {
			if(child.GetComponent<Image>() != null) {
				SetImageAlpha(child.GetComponent<Image>(), value);
			}
			else if(child.GetComponent<Text>() != null) {
				SetTextAlpha(child.GetComponent<Text>(), value);
			}
		}
	}

	// Used to edit the alpha of an image
	public static void SetImageAlpha(Image image, float value) {
		Color tempColor = image.color;
		tempColor.a = value;
		image.color = tempColor;
	}
	// Used to edit the alpha of a text
	public static void SetTextAlpha(Text text, float value) {
